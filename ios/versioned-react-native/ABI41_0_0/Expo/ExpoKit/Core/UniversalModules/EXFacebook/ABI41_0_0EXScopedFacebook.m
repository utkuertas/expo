...
@interface ABI41_0_0EXFacebook (ExportedMethods)
...
- (void)logOutAsync:(ABI41_0_0UMPromiseResolveBlock)resolve
           rejecter:(ABI41_0_0UMPromiseRejectBlock)reject;

- (void)getAuthenticationCredentialAsync:(ABI41_0_0UMPromiseResolveBlock)resolve
                   rejecter:(ABI41_0_0UMPromiseRejectBlock)reject;
...
@interface ABI41_0_0EXScopedFacebook ()
...
// Expo Go-only ABI41_0_0EXFacebook module, which ensures that Facebook SDK configurations
...
{
  if (self = [super init]) {
    NSString *suiteName = [NSString stringWithFormat:@"%@#%@", NSStringFromClass(self.class), experienceScopeKey];
    ...
    if (hasPreviouslySetAutoInitEnabled || manifestDefinesAutoInitEnabled) {
      ...
      // the mechanism behind ABI41_0_0EXFacebookAppDelegate.
      // Check for FacebookAppId in case this is a custom client build
      if (scopedFacebookAppId) {
        [FBSDKApplicationDelegate initializeSDK:nil];
        _isInitialized = YES;
        if (manifestFacebookAppId) {
          ABI41_0_0UMLogInfo(@"Overriding Facebook App ID with Expo Go's. To test your own Facebook App ID, you'll need to build a standalone app. Refer to our documentation for more info- https://docs.expo.io/versions/latest/sdk/facebook/");
        }
      } else {
        ABI41_0_0UMLogWarn(@"FacebookAutoInit is enabled, but no FacebookAppId has been provided. Facebook SDK initialization aborted.");
      }
    }
  }
  return self;
...
- (void)setAutoInitEnabled:(BOOL)enabled resolver:(ABI41_0_0UMPromiseResolveBlock)resolve rejecter:(ABI41_0_0UMPromiseRejectBlock)reject
{
  if (enabled) {
...
- (void)logInWithReadPermissionsWithConfig:(NSDictionary *)config resolver:(ABI41_0_0UMPromiseResolveBlock)resolve rejecter:(ABI41_0_0UMPromiseRejectBlock)reject
{
...
- (void)getAuthenticationCredentialAsync:(ABI41_0_0UMPromiseResolveBlock)resolve rejecter:(ABI41_0_0UMPromiseRejectBlock)reject
{
  // If the developer didn't initialize the SDK, let them know.
  if (!_isInitialized) {
    reject(@"ERR_FACEBOOK_UNINITIALIZED", @"Facebook SDK has not been initialized yet.", nil);
...
- (void)logOutAsync:(ABI41_0_0UMPromiseResolveBlock)resolve rejecter:(ABI41_0_0UMPromiseRejectBlock)reject
{
  // If the developer didn't initialize the SDK, let them know.
  if (!_isInitialized) {
    reject(@"ERR_FACEBOOK_UNINITIALIZED", @"Facebook SDK has not been initialized yet.", nil);
