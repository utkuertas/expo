...
- (instancetype)initWithExperienceScopeKey:(NSString *)experienceScopeKey andConstantsBinding:(ABI41_0_0EXConstantsBinding *)constantsBinding
{
  if (self = [super init]) {
...
// overriding ABI41_0_0EXPermission to inject scoped permission logic
...
- (NSString *)getScopedPermissionStatus:(NSString *)permissionType {
  ...
  return [[self class] permissionStringForStatus:[_permissionsService getPermission:permissionType forExperience:_experienceScopeKey]];
}
...
- (BOOL)hasGrantedScopedPermission:(NSString *)permissionType
{
  ...
  return [_permissionsService getPermission:permissionType forExperience:_experienceScopeKey] == ABI41_0_0UMPermissionStatusGranted;
}
  ...
  if (![globalPermissions[@"status"] isEqualToString:@"granted"]) {
    ...
    void (^customOnResults)(NSDictionary *) = ^(NSDictionary *permission){
      ...
      if ([self shouldVerifyScopedPermission:permissionType]) {
        [self.permissionsService savePermission:permission ofType:permissionType forExperience:self.experienceScopeKey];
      }
      resolve(permission);
    };
    ...
    UIAlertAction *allowAction = [UIAlertAction actionWithTitle:@"Allow" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
      ...
      if (![self.permissionsService savePermission:permission ofType:permissionType forExperience:self.experienceScopeKey]) {
...
{
  NSString *experienceName = self.experienceScopeKey; // TODO: we might want to use name from the manifest?
