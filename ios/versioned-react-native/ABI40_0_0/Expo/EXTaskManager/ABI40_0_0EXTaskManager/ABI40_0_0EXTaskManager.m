...
NSString * const ABI40_0_0EXTaskManagerEventName = @"TaskManager.executeTask";

@interface ABI40_0_0EXTaskManager ()
...
@property (nonatomic, weak) id<ABI40_0_0UMTaskServiceInterface> taskService;

@end

@implementation ABI40_0_0EXTaskManager

ABI40_0_0UM_EXPORT_MODULE(ExpoTaskManager);

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(ABI40_0_0UMTaskManagerInterface)];
}

- (instancetype)init
{
  return [self initWithExperienceScopeKey:@"mainApplication"];
}
...
{
  return @{
           @"EVENT_NAME": ABI40_0_0EXTaskManagerEventName,
           };
}
...
{
  return @[ABI40_0_0EXTaskManagerEventName];
}
...
- (void)startObserving
{
  if (_eventsQueue && _eventsQueue.count > 0) {
    ...
    for (NSDictionary *eventBody in _eventsQueue) {
      [_eventEmitter sendEventWithName:ABI40_0_0EXTaskManagerEventName body:eventBody];
    }
  }
  _eventsQueue = nil;
}
...
ABI40_0_0UM_EXPORT_METHOD_AS(isAvailableAsync,
                    isAvailable:(ABI40_0_0UMPromiseResolveBlock)resolve
                    rejecter:(ABI40_0_0UMPromiseRejectBlock)reject)
{
  resolve(@(_taskService != nil));
}

ABI40_0_0UM_EXPORT_METHOD_AS(notifyTaskFinishedAsync,
...
ABI40_0_0UM_EXPORT_METHOD_AS(isTaskRegisteredAsync,
...
ABI40_0_0UM_EXPORT_METHOD_AS(getRegisteredTasksAsync,
                    getRegisteredTasks:(ABI40_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI40_0_0UMPromiseRejectBlock)reject)
{
  resolve([_taskService getRegisteredTasksForAppId:_appId]);
}

ABI40_0_0UM_EXPORT_METHOD_AS(getTaskOptionsAsync,
                    getConfigurationForTaskName:(nonnull NSString *)taskName
                    resolve:(ABI40_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI40_0_0UMPromiseRejectBlock)reject)
{
  resolve(ABI40_0_0UMNullIfNil([_taskService getOptionsForTaskName:taskName forAppId:_appId]));
}
...
ABI40_0_0UM_EXPORT_METHOD_AS(unregisterAllTasksAsync,
                    unregisterAllTasks:(ABI40_0_0UMPromiseResolveBlock)resolve
                    reject:(ABI40_0_0UMPromiseRejectBlock)reject)
{
...
# pragma mark - ABI40_0_0UMTaskManagerInterface
...
{
  return [ABI40_0_0EXTaskService hasBackgroundModeEnabled:backgroundMode];
}
...
- (void)executeWithBody:(NSDictionary *)body
{
  if (!_eventsQueue) {
    ...
    [_eventEmitter sendEventWithName:ABI40_0_0EXTaskManagerEventName body:body];
  } else {
...
{
  // TODO(@tsapeta): find app url for vanilla ABI40_0_0RN apps
